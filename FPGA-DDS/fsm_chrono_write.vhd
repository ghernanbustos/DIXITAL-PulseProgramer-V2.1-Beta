--------------------------------------------------------------------------------
--	  Autor: 			  Guillermo H. Bustos
--   FileName:         fsm_chrono_write.vhd
--   Dependencies:     none
--   Design Software:  Quartus Prime Lite Edition
--	  Working place:    Universidad Nacional de Cordoba - FAMAF - LARTE
--   Project: 			  Dixital 
--	  Company: 			  Trovintek
--   Version History
--   Version 1.0 03/2023 Guillermo H. Bustos
--
--	  Brief:
--		FSM controls two external blocks, chrono module and ram module. Fetch pre loaded data from ram to set
--			precount value in chrono module, and then enable to start counting. 
--		fsm remains in last state until external asynchronous reset is set. to avoid re-init if all finish when
-- 		start signal remaining high. Avoid undesired initialization after execution ending.
-- 	state_A es extra for right pre-load data in chrono.
-- 	FSM set end flags at last state, in case of chrono, C polling routine in HPS remains 
-- 		steady expecting hight state, this cause firmware not continuous excecution until 
--			chrono ends. Take it in account when using polling to read flags. 
--------------------------------------------------------------------------

library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;

ENTITY fsm_chrono_write IS
  port (
		clk_fsm_chrono    	: 	IN  std_logic; -- clk_fsm_chrono 50Mhz
		rst_n_fsm_chrono  	: 	IN  std_logic;
		start_fsm_chrono		: 	IN  std_logic; -- set on with wrapper CtrlRegister
		AddressMem_offset		:	IN  std_logic_vector(5 downto 0);
		-- From Chrono
		overflowFlag			: 	IN  std_logic; -- chrono ends  overflow flag.
		-- FSM RAM control
		rd_CHRMem				: 	OUT std_logic; 
		wr_CHRMem 				: 	OUT std_logic; 
		en_CHRMem 				: 	OUT std_logic; 
		clc_CHRMem				: 	OUT std_logic;
		addressChronoMem		:	OUT std_logic_vector(5 downto 0); 
		-- fsm end
		fsm_chronoEND			:	OUT std_logic;
		-- Chrono control
		nRst_chrono 			: 	OUT  std_logic;
		en_chrono   			: 	OUT  std_logic;
		setPrecount_chrono	: 	OUT  std_logic;
		s_init,s_extra,s0,s1,s1_bis,s2,s3  : 	OUT  std_logic
		
  );
END ENTITY fsm_chrono_write;
------------------------------------------------


ARCHITECTURE Behavioral OF fsm_chrono_write IS
  -- Signals declarations -- state_1_bis extra state to help prescaler signal be registered
  TYPE state_type IS (initialState,state_extra, state_0, state_1, state_2, state_2_bis,state_3); 
  SIGNAL state : state_type; 														 
  
BEGIN
state_transition: PROCESS (clk_fsm_chrono, rst_n_fsm_chrono) 	 
	BEGIN
		IF (rst_n_fsm_chrono = '0') THEN   							
			state <= initialState;	
			addressChronoMem <= "000000"; -- ensure zero in rst state
			
		ELSIF rising_edge(clk_fsm_chrono) THEN 
			CASE state IS
				WHEN initialState => -- register offset mem input
					IF (start_fsm_chrono = '1') THEN 
						state <= state_extra;	
					ELSE 
						state <= initialState; 
					END IF;
	
				WHEN state_extra =>		-- fsm prescaler record	 
					addressChronoMem <= AddressMem_offset;
					state <= state_0; 	
					
				WHEN state_0 =>		-- Mem enable		 
					state <= state_1; 	
					
				WHEN state_1 =>		-- data mem output
					state <= state_2;
					
				WHEN state_2 =>		-- data mem output
					state <= state_2_bis;
					
				WHEN state_2_bis =>		-- data mem output
					IF (overflowFlag = '1') THEN
						state <= state_3;
					ELSE 
						state <= state_2_bis; -- remains counting
					END IF;
				WHEN state_3 =>		--  chrono off
					state <= state_3;					
			END CASE;
		END IF;
	END PROCESS state_transition;
  
  
Output_logic: PROCESS (state)
	BEGIN							
 		CASE state IS
			WHEN initialState => -- RAM clear/ LOAD OFFSET
				-- FSM RAM control
				rd_CHRMem				<= '0';
				wr_CHRMem 				<= '0';
				en_CHRMem 				<= '0';
				clc_CHRMem				<= '1'; -- check this port
				-- chrono
				nRst_chrono 			<= '0'; --rst off
				en_chrono   			<= '0';
				setPrecount_chrono	<= '0';
				-- endflag
				fsm_chronoEND			<= '0';

				s_init 		<= '1';
				s_extra 		<= '0';
				s0 			<= '0';
				s1 			<= '0';
				s1_bis 		<= '0';
				s2 			<= '0';
				s3 			<= '0';
			WHEN state_extra => -- LOAD OFFSET fsm / latch mem offset
				-- FSM RAM control
				rd_CHRMem				<= '0';
				wr_CHRMem 				<= '0';
				en_CHRMem 				<= '0';
				clc_CHRMem				<= '0'; -- check this port
				-- chrono
				nRst_chrono 			<= '0'; --rst off
				en_chrono   			<= '0';
				setPrecount_chrono	<= '0';
				-- endflag
				fsm_chronoEND			<= '0';
								
				s_init 		<= '0';
				s_extra 		<= '1';
				s0 			<= '0';
				s1 			<= '0';
				s1_bis 		<= '0';
				s2 			<= '0';
				s3 			<= '0';

			WHEN state_0 => -- MEM EN with address offset
				rd_CHRMem				<= '1';
				wr_CHRMem 				<= '0';
				en_CHRMem 				<= '1';
				clc_CHRMem				<= '0';
				-- chrono
				nRst_chrono 			<= '1'; 
				en_chrono   			<= '0'; -- chrono reset
				setPrecount_chrono	<= '0';
				-- endflag
				fsm_chronoEND			<= '0';
				
				s_init 		<= '0';
				s_extra 		<= '0';
				s0 			<= '1';
				s1 			<= '0';
				s1_bis 		<= '0';
				s2 			<= '0';
				s3 			<= '0';

			WHEN state_1 => -- Set Chrono pre Load 
				-- ram control
				rd_CHRMem				<= '1';
				wr_CHRMem 				<= '0';
				en_CHRMem 				<= '1';
				clc_CHRMem				<= '0';
				-- chrono
				nRst_chrono 			<= '1'; -- chrono reset OFF
				en_chrono   			<= '0';
				setPrecount_chrono	<= '1'; -- preload on once
				-- endflag
				fsm_chronoEND			<= '0';	
				
				s_init 		<= '0';
				s_extra 		<= '0';
				s0 			<= '0';
				s1 			<= '1';
				s1_bis 		<= '0';
				s2 			<= '0';
				s3 			<= '0';
				
			WHEN state_2 => -- Set Chrono pre Load 
				-- ram control
				rd_CHRMem				<= '1';
				wr_CHRMem 				<= '0';
				en_CHRMem 				<= '1';
				clc_CHRMem				<= '0';
				-- chrono
				nRst_chrono 			<= '1'; -- chrono reset OFF
				en_chrono   			<= '1';
				setPrecount_chrono	<= '0'; -- preload on once
				-- endflag
				fsm_chronoEND			<= '0';	
				
				s_init 		<= '0';
				s_extra 		<= '0';
				s0 			<= '0';
				s1 			<= '1';
				s1_bis 		<= '0';
				s2 			<= '0';
				s3 			<= '0';
				
			WHEN state_2_bis => -- Enable Chrono count
				-- ram control
				rd_CHRMem				<= '1';
				wr_CHRMem 				<= '0';
				en_CHRMem 				<= '1';
				clc_CHRMem				<= '0';
				-- chrono
				nRst_chrono 			<= '1'; -- chrono reset OFF
				en_chrono   			<= '1';
				setPrecount_chrono	<= '0'; -- preload on once
				-- endflag
				fsm_chronoEND			<= '0';	
					
				s_init 		<= '0';
				s_extra 		<= '0';
				s0 			<= '0';
				s1 			<= '0';
				s1_bis 		<= '1';
				s2 			<= '0';
				s3 			<= '0';
				
			WHEN state_3 => -- Chrono reset and fsm waits for HPS external rst signal
				-- ram control
				rd_CHRMem				<= '1';
				wr_CHRMem 				<= '0';
				en_CHRMem 				<= '1';
				clc_CHRMem				<= '0';
				-- chrono
				nRst_chrono 			<= '0'; --rst chrono
				en_chrono   			<= '0';
				setPrecount_chrono	<= '0';
				-- endflag
				fsm_chronoEND			<= '1'; --fsm ends flag
					
				s_init 		<= '0';
				s_extra 		<= '0';
				s0 			<= '0';
				s1 			<= '0';
				s1_bis 		<= '0';
				s2 			<= '0';
				s3 			<= '1';

		END CASE;
	END PROCESS Output_logic; -- Outputs are updated at the end of the process

END ARCHITECTURE Behavioral;