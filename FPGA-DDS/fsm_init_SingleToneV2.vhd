...
state_transition: PROCESS (clk_fsm_SingleTone, rst_n_fsm_SingleTone, start_fsm_SingleTone) 	--sensitive list
	...
	BEGIN
		IF (rst_n_fsm_SingleTone = '0') THEN   -- asynchronous reset
			...
			CASE state IS
				...
				WHEN state_1 =>	-- Master Reset Time ON > 10 dds_clk system cycles 			
	...
	-- Load register configuration from PRE LOADED BUFFER--------------------------
				...
				WHEN state_3 =>	-- DDS writes								
					count_AddressMem := count_AddressMem + 1;
					state <= state_4;
					
				WHEN state_4 => -- change mem address
					AddressMemInit <= std_logic_vector(to_unsigned(count_AddressMem, AddressMemInit'length));	
...
--			WHEN state_2 => -- clear mem output CHECK TO ERASE
--			-- FPGA internal RAM control
--				rd_InitMem		<= '0';	
--				wr_InitMem		<= '0';	
--				en_InitMem		<= '0';
--				clc_InitMem 	<= '0';	-- clear mem output
--			-- DDS Write Data Control	
