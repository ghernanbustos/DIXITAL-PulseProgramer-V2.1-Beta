-- Copyright (C) 2023  Intel Corporation. All rights reserved.
-- Your use of Intel Corporation's design tools, logic functions 
-- and other software and tools, and any partner logic 
-- functions, and any output files from any of the foregoing 
-- (including device programming or simulation files), and any 
-- associated documentation or information are expressly subject 
-- to the terms and conditions of the Intel Program License 
-- Subscription Agreement, the Intel Quartus Prime License Agreement,
-- the Intel FPGA IP License Agreement, or other applicable license
-- agreement, including, without limitation, that your use is for
-- the sole purpose of programming logic devices manufactured by
-- Intel and sold by Intel or its authorized distributors.  Please
-- refer to the applicable agreement for further details, at
-- https://fpgasoftware.intel.com/eula.

-- *****************************************************************************
-- This file contains a Vhdl test bench with test vectors .The test vectors     
-- are exported from a vector file in the Quartus Waveform Editor and apply to  
-- the top level entity of the current Quartus project .The user can use this   
-- testbench to simulate his design using a third-party simulation tool .       
-- *****************************************************************************
-- Generated on "10/30/2024 16:44:32"
                                                             
-- Vhdl Test Bench(with test vectors) for design  :          fsm_ftw_writeV1_1
-- 
-- Simulation tool : 3rd Party
-- 

LIBRARY ieee;                                               
USE ieee.std_logic_1164.all;                                

ENTITY fsm_ftw_writeV1_1_vhd_vec_tst IS
END fsm_ftw_writeV1_1_vhd_vec_tst;
ARCHITECTURE fsm_ftw_writeV1_1_arch OF fsm_ftw_writeV1_1_vhd_vec_tst IS
-- constants                                                 
-- signals                                                   
SIGNAL AddressMem_offset : STD_LOGIC_VECTOR(5 DOWNTO 0);
SIGNAL addressMemFTW : STD_LOGIC_VECTOR(5 DOWNTO 0);
SIGNAL clc_FTWMem : STD_LOGIC;
SIGNAL clk_fsm_ftw : STD_LOGIC;
SIGNAL en_FTWMem : STD_LOGIC;
SIGNAL flag1 : STD_LOGIC;
SIGNAL flag2 : STD_LOGIC;
SIGNAL flag3 : STD_LOGIC;
SIGNAL flag4 : STD_LOGIC;
SIGNAL flag5 : STD_LOGIC;
SIGNAL flag6 : STD_LOGIC;
SIGNAL FSK_BPSK_HOLD_dds : STD_LOGIC;
SIGNAL fsm_ftwEND : STD_LOGIC;
SIGNAL I_O_update_dds : STD_LOGIC;
SIGNAL masterRst_dds : STD_LOGIC;
SIGNAL rd_FTWMem : STD_LOGIC;
SIGNAL rd_n_dds : STD_LOGIC;
SIGNAL rst_n_fsm_ftw : STD_LOGIC;
SIGNAL start_fsm_ftw : STD_LOGIC;
SIGNAL wd_n_dds : STD_LOGIC;
SIGNAL wr_FTWMem : STD_LOGIC;
COMPONENT fsm_ftw_writeV1_1
	PORT (
	AddressMem_offset : IN STD_LOGIC_VECTOR(5 DOWNTO 0);
	addressMemFTW : OUT STD_LOGIC_VECTOR(5 DOWNTO 0);
	clc_FTWMem : OUT STD_LOGIC;
	clk_fsm_ftw : IN STD_LOGIC;
	en_FTWMem : OUT STD_LOGIC;
	flag1 : OUT STD_LOGIC;
	flag2 : OUT STD_LOGIC;
	flag3 : OUT STD_LOGIC;
	flag4 : OUT STD_LOGIC;
	flag5 : OUT STD_LOGIC;
	flag6 : OUT STD_LOGIC;
	FSK_BPSK_HOLD_dds : OUT STD_LOGIC;
	fsm_ftwEND : OUT STD_LOGIC;
	I_O_update_dds : OUT STD_LOGIC;
	masterRst_dds : OUT STD_LOGIC;
	rd_FTWMem : OUT STD_LOGIC;
	rd_n_dds : OUT STD_LOGIC;
	rst_n_fsm_ftw : IN STD_LOGIC;
	start_fsm_ftw : IN STD_LOGIC;
	wd_n_dds : OUT STD_LOGIC;
	wr_FTWMem : OUT STD_LOGIC
	);
END COMPONENT;
BEGIN
	i1 : fsm_ftw_writeV1_1
	PORT MAP (
-- list connections between master ports and signals
	AddressMem_offset => AddressMem_offset,
	addressMemFTW => addressMemFTW,
	clc_FTWMem => clc_FTWMem,
	clk_fsm_ftw => clk_fsm_ftw,
	en_FTWMem => en_FTWMem,
	flag1 => flag1,
	flag2 => flag2,
	flag3 => flag3,
	flag4 => flag4,
	flag5 => flag5,
	flag6 => flag6,
	FSK_BPSK_HOLD_dds => FSK_BPSK_HOLD_dds,
	fsm_ftwEND => fsm_ftwEND,
	I_O_update_dds => I_O_update_dds,
	masterRst_dds => masterRst_dds,
	rd_FTWMem => rd_FTWMem,
	rd_n_dds => rd_n_dds,
	rst_n_fsm_ftw => rst_n_fsm_ftw,
	start_fsm_ftw => start_fsm_ftw,
	wd_n_dds => wd_n_dds,
	wr_FTWMem => wr_FTWMem
	);

-- clk_fsm_ftw
t_prcs_clk_fsm_ftw: PROCESS
BEGIN
LOOP
	clk_fsm_ftw <= '0';
	WAIT FOR 10000 ps;
	clk_fsm_ftw <= '1';
	WAIT FOR 10000 ps;
	IF (NOW >= 1000000 ps) THEN WAIT; END IF;
END LOOP;
END PROCESS t_prcs_clk_fsm_ftw;

-- rst_n_fsm_ftw
t_prcs_rst_n_fsm_ftw: PROCESS
BEGIN
	rst_n_fsm_ftw <= '1';
WAIT;
END PROCESS t_prcs_rst_n_fsm_ftw;

-- start_fsm_ftw
t_prcs_start_fsm_ftw: PROCESS
BEGIN
	start_fsm_ftw <= '0';
	WAIT FOR 60000 ps;
	start_fsm_ftw <= '1';
	WAIT FOR 20000 ps;
	start_fsm_ftw <= '0';
WAIT;
END PROCESS t_prcs_start_fsm_ftw;
-- AddressMem_offset[5]
t_prcs_AddressMem_offset_5: PROCESS
BEGIN
	AddressMem_offset(5) <= '0';
WAIT;
END PROCESS t_prcs_AddressMem_offset_5;
-- AddressMem_offset[4]
t_prcs_AddressMem_offset_4: PROCESS
BEGIN
	AddressMem_offset(4) <= '0';
WAIT;
END PROCESS t_prcs_AddressMem_offset_4;
-- AddressMem_offset[3]
t_prcs_AddressMem_offset_3: PROCESS
BEGIN
	AddressMem_offset(3) <= '0';
WAIT;
END PROCESS t_prcs_AddressMem_offset_3;
-- AddressMem_offset[2]
t_prcs_AddressMem_offset_2: PROCESS
BEGIN
	AddressMem_offset(2) <= '1';
WAIT;
END PROCESS t_prcs_AddressMem_offset_2;
-- AddressMem_offset[1]
t_prcs_AddressMem_offset_1: PROCESS
BEGIN
	AddressMem_offset(1) <= '1';
WAIT;
END PROCESS t_prcs_AddressMem_offset_1;
-- AddressMem_offset[0]
t_prcs_AddressMem_offset_0: PROCESS
BEGIN
	AddressMem_offset(0) <= '0';
WAIT;
END PROCESS t_prcs_AddressMem_offset_0;
END fsm_ftw_writeV1_1_arch;
