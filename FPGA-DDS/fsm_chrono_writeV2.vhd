...
--   FileName:         fsm_chrono_writeV2.vhd
--   Dependencies:     none
...
-- 	nco_rst used for reset NCO before windows rf is on. This is only used when chrono is sync 
-- 		with NCO Sinc pulse.
--------------------------------------------------------------------------
...
ENTITY fsm_chrono_writeV2 IS
  port (
		...
		--
		nco_rst					: 	OUT  std_logic -- to control NCO when rfwindow controls it
		
		
  );
END ENTITY fsm_chrono_writeV2;
------------------------------------------------


ARCHITECTURE Behavioral OF fsm_chrono_writeV2 IS
...
--  TYPE state_type IS (initialState, state_0, state_1, state_2, state_3, state_4, state_5, state_6, state_7); 
	TYPE state_type IS (initialState, state_0, state_1, state_3, state_4, state_7); 
...
--			WHEN state_5 => -- Chrono prescaler OFF / chrono on
--				-- ram control
--				rd_CHRMem				<= '1';
--				wr_CHRMem 				<= '0';
--				en_CHRMem 				<= '1';
