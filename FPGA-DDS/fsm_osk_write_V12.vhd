...
--   FileName:         fsm_osk_write_V12.vhd
--   Dependencies:     none
...
--
--	  Brief: 
-- 	fetch amplitude data from an embedded NCO and writes DDS amplitud registers to
--		produce an amplitude modulation in both chanells.
-- 	This fsm reinitiate itself, It doesnt expect for an external reset.
...
-- 	10 fsm states, so for a clock_50 there is 200nS of lag
-- 	When NCO module is disable, DDS needs to have max amplitud in I and Q chanells
-- 	this fsm needs extra code to do that
-- 	All the process occurs before the next NCO clock rising edge occurs, so this fsm should do all on time
--------------------------------------------------------------------------------
...
END ENTITY fsm_osk_write_V12;
------------------------------------------------
ARCHITECTURE Behavioral OF fsm_osk_write_V12 IS
  -- Signals declarations
	TYPE state_type IS (state_init1, state_latchData, state_0, state_1, state_2, state_3,state_4, state_5, state_6, state_7, state_8);	
		...
		ELSIF rising_edge(osk_clk) THEN -- Change State
			CASE state IS
				...
				WHEN state_latchData => -- load 12 amplitude data and format to 16bits in aux16bits
						aux16bits <= "0000" & osk_12bitDataIn; -- splitted in two 8 bit words
						state <= state_0;
...
-- write Q channel registers		( Let chanell without changes in amplitude)				
				WHEN state_4 =>			-- 16bit word [Q_DirReg + 8_MSB osk_12bit word]
						osk_16bitDataOut <= regDir8_Q_msb & "00001111";				
						state <= state_5;
				...
				WHEN state_6 =>			-- 16bit word [Q_DirReg + 8_MSB osk_12bit word]
						osk_16bitDataOut <= regDir8_Q_lsb & "11111111";				
						state <= state_7;
...
Output_logic: PROCESS (state) -- when state changes this process is active
	BEGIN							
 		CASE state IS
			...
			WHEN state_latchData =>		-- reset ff_d to expect for the next 1mhz clock rising edge
				-- DDS Write Data Control	
				dds_masterRst		<= '0';	 			
				dds_rd_n				<= '1';				 	 	
				dds_wr_n				<= '1';				 		
