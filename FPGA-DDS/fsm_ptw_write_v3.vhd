...
END ENTITY fsm_ptw_write_v3;
  
ARCHITECTURE Behavioral OF fsm_ptw_write_v3 IS
  -- Signals declarations
  TYPE state_type IS (initialState, initialStateBis, state_1, state_2, state_3, state_4, state_5);	-- Enumerated type decl
  SIGNAL state : state_type; 											-- state_type is signal type

BEGIN
state_transition: PROCESS (clk_fsm_ptw, rst_n_fsm_ptw) 	--sensitive list
  	-- Variables declarations
	VARIABLE ptw_byte_num : INTEGER RANGE 0 TO 6; -- 2 words, 16 bits each one to complete 16 bit PTW
		...
		ELSIF rising_edge(clk_fsm_ptw) THEN -- Change State
			CASE state IS
				...
				WHEN initialStateBis => -- Latch offset address
						...
						count_limit		  := ( to_integer(unsigned(AddressMem_offset)) + ptw_byte_num ); 
				...
				WHEN state_2 => -- data ram output 
						state <= state_3;
						
				WHEN state_3 => -- count_AddressMem ++ | 					
				...
				WHEN state_4 => -- update dds
						state <= state_5;
						
				WHEN state_5 => -- waits for reset signal
						state <= state_5;	
...
Output_logic: PROCESS (state)
	BEGIN							
 		CASE state IS
			...
			WHEN initialStateBis => -- Latch offset
					-- FPGA internal RAM control
				rd_PTWMem			<= '0';			
				wr_PTWMem			<= '0';	 			 						
				en_PTWMem			<= '0';
			...
			WHEN state_1 => -- ENABLE MEM + MEM OFFSET
					-- FPGA internal RAM control
