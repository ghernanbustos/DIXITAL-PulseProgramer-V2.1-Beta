...
--		LUT  to be used in DDS as an amplitud modulation				  --
--		Set a flag_clkout signal for triggering. nco_enable signal
--		add reset in the second process..
-- 								06.03.2026
...
port (
		  ...
		  -- Debugging signal
		  lut_addr_db : out std_logic_vector(11 downto 0)
  );
  
end entity;


architecture rtl of waveform_gen_V3 is
---------------------------------------------------------
		component sincos_lut_v3
			port (
...
---------------------------------------------------------	
	...
	phase_acc_reg: process(clk, reset_n)
		begin
		  if reset_n = '0' then
			phase_acc 	<= (others => '0');
		  elsif ( rising_edge(clk) and nco_enable = '1') then
	...
	---------------------------------------------------------------------
	-- Replicate clock signal in the output to be used as a fsm trigger 
	---------------------------------------------------------------------
	flag_clkout <= clk;
	----------------------------------------------------------------------
	...
	delay_regs: process(clk,reset_n)
		begin
		  if reset_n = '0' then
			 ...
			 lut_addr_db <= (others => '0');
		  elsif (rising_edge(clk) and nco_enable = '1')  then
			 lut_addr_reg <= lut_addr;
			 lut_addr_db <= lut_addr;
		  end if;
		end process delay_regs;
		---------------------------------------------
		-- Square output is msb of the accumulator --
		---------------------------------------------
		squ_out <= "100000000000"
				when lut_addr_reg(11) = '1' 
					else "011111111111";
		-------------------------------------------------------
