...
--				Prestar atencion y revisar que a la FFT no le sobre un estado al vicio (trama_out)
--------------------------------------------------------------------------------
	LIBRARY ieee;
	USE ieee.std_logic_1164.all;
	-----------------------------------------------------
	ENTITY fsm_spidebugging IS --dp: data path
		PORT ( 
	...
	END fsm_spidebugging;
	-----------------------------------------------------
	ARCHITECTURE comportamental OF fsm_spidebugging IS
	TYPE state_type IS (state0, state1,state_debug1,state_debug2, state2, state3, state4);
	...
	BEGIN								 -- codigo SECUENCIAL estamos en presencia de un PROCESS
 		...
 		CASE state IS
			...
			WHEN state3 =>
				rx_req		 <= '0';				
				ff_reg_en	 <= '0'; 
				cont_en		 <= '1'; -- Habilito el contador para que decremente con el clock_50
				buff_en	    <= '0';  
			WHEN state4 =>
				rx_req		 <= '0';				
				ff_reg_en	 <= '0'; 
