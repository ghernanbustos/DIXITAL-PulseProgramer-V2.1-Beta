...
architecture behavioral of ADC_spi_master is
	 ...
	 type t_State is (init_state, tx_state); 	-- define type of data
	 signal State : t_State;						-- declare
...
begin
	...
	begin
		if rising_edge(ADC_clk) then
			...
			case State is
				when init_state =>
					ADC_CONVST 			<= '1'; -- conversion period
					ADC_DIN 				<= '0';
					sampling_active	<= '0';
					done 					<= '0';
					config_counter 	<=   0;
					if i = 60 then 
						State <= tx_state; -- next transition from 60 cycle
					end if;
					
				when tx_state =>
					ADC_CONVST <= '0'; -- out of conversion period
					case i is
						when 62 => 
							ADC_DIN <= config_bits(17);
						when 63 => 
							ADC_DIN <= config_bits(16);
						when 64 => 
							ADC_DIN <= config_bits(15);
						when 65 => 
							ADC_DIN <= config_bits(14);
						when 66 => 
							ADC_DIN <= config_bits(13);
						when 67 => 
							ADC_DIN <= config_bits(12);
						when 68 => 
							ADC_DIN <= config_bits(11);
